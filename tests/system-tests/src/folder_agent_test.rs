...
async fn folder_agent_recovers_after_crash_during_active_sync_writes() -> Result<()> {
    ...
    let result = async {
        ...
        let status = timeout(Duration::from_secs(30), crash_run.status())
            .await
            .context("folder-agent active-write crash injection timed out")?
            .context("failed to execute folder-agent active-write crash injection")?;
        if status.success() {
            bail!(
                "expected folder-agent active-write crash injection to fail, got status {status}"
            );
        }
