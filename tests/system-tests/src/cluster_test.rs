...
mod tests {
    ...
    #[tokio::test]
    async fn bootstrap_claim_enrolled_client_roundtrips_with_auth_required_node() -> Result<()> {
        let rendezvous_bind = "127.0.0.1:19225";
        let bind = "127.0.0.1:19226";
        let cluster_id = "11111111-1111-7111-8111-111111111126";
        let node_id = "00000000-0000-0000-0000-000000000926";
        let admin_token = "claim-roundtrip-secret";

        let rendezvous_url = format!("http://{rendezvous_bind}");
        let base_url = format!("http://{bind}");
        let data_dir = fresh_data_dir("bootstrap-claim-roundtrip-auth-node");
        ...
        let result = async {
            ...
            let enrolled = issue_bootstrap_claim_and_enroll_client(
                &http,
                &base_url,
                admin_token,
                &client_dir,
                "claim-roundtrip.claim.json",
                "claim-roundtrip.bootstrap.json",
                Some("claim-roundtrip-client"),
                Some(3600),
                None,
            )
            .await?;
            let client = enrolled.build_client_async().await?;
            let key = "claim-roundtrip.txt";
            let payload = Bytes::from_static(b"hello from claim-enrolled client");
