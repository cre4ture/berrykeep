...
const BerryKeepIndicator = GObject.registerClass(
class BerryKeepIndicator extends PanelMenu.Button {
    _init(extension) {
        super._init(0.0, 'BerryKeep Status', false);
        this.add_style_class_name('berrykeep-indicator');
        ...
        this._iconBox = new St.BoxLayout({
            style_class: 'berrykeep-panel-icons',
        });
        this._brandIcon = new St.Icon({
            gicon: this._loadBrandIcon(),
            icon_size: 15,
            style_class: 'berrykeep-brand-icon',
        });
        this._statusIcon = new St.Icon({
            icon_name: 'dialog-question-symbolic',
            icon_size: 16,
            style_class: 'system-status-icon berrykeep-status-icon',
        });
    ...
    _loadBrandIcon() {
        const iconPath = GLib.build_filenamev([
            ...
            'icons',
            'berrykeep-brand-symbolic.svg',
        ]);
        const iconFile = Gio.File.new_for_path(iconPath);
        if (!iconFile.query_exists(null)) {
            throw new Error(`BerryKeep brand icon is missing at ${iconPath}`);
        }
        return new Gio.FileIcon({file: iconFile});
    }

    _buildMenu() {
        const header = new PopupMenu.PopupMenuItem('BerryKeep', {
            reactive: false,
            can_focus: false,
        });
        header.label.add_style_class_name('berrykeep-header');
        ...
        this._openConfigItem = new PopupMenu.PopupMenuItem('Open BerryKeep Config');
    ...
    _addStatusRow(title) {
        ...
        item.add_style_class_name('berrykeep-status-row');
        ...
        const titleLabel = new St.Label({
            text: title,
            style_class: 'berrykeep-status-title',
        });
        const valueLabel = new St.Label({
            text: 'Waiting for BerryKeep',
            style_class: 'berrykeep-status-value',
        });
        const detailLabel = new St.Label({
            text: '',
            style_class: 'berrykeep-status-detail',
        });
    ...
    _startMonitoring() {
        try {
            ...
            logError(error, 'BerryKeep Status: failed to monitor status file');
        }
    ...
    _reloadStatus() {
        try {
            ...
            if (!ok) {
                this._applyMissingState('Waiting for BerryKeep config app', 'Status file could not be loaded');
                return;
            }
            ...
            if (this._isStale(payload)) {
                this._applyMissingState(
                    'Waiting for live BerryKeep updates',
            ...
            if (error.matches?.(Gio.IOErrorEnum, Gio.IOErrorEnum.NOT_FOUND)) {
                this._applyMissingState('Waiting for BerryKeep config app', 'Start berrykeep-config-app to publish desktop status');
                return;
            }

            logError(error, 'BerryKeep Status: failed to reload status');
            this._applyMissingState('BerryKeep status error', `${error}`);
        }
    }
    ...
    _applyPayload(payload) {
        ...
        this._targetRow.valueLabel.text = payload?.profileLabel ?? 'BerryKeep';
    ...
    _applyFacet(row, facet) {
        ...
        row.item.remove_style_class_name('berrykeep-state-error');
        row.item.remove_style_class_name('berrykeep-state-warning');
        row.item.remove_style_class_name('berrykeep-state-syncing');

        switch (facet?.state) {
        case 'error':
            row.item.add_style_class_name('berrykeep-state-error');
            break;
        case 'warning':
            row.item.add_style_class_name('berrykeep-state-warning');
            break;
        case 'syncing':
        case 'starting':
            row.item.add_style_class_name('berrykeep-state-syncing');
            break;
        default:
            break;
        }
    ...
    _openConfigApp() {
        ...
        try {
            ...
            logError(error, 'BerryKeep Status: failed to open config app');
        }
    }
    ...
    _applyMissingState(summary, detail) {
        ...
        this._applyFacet(this._connectionRow, {
            ...
            detail: 'Waiting for the BerryKeep config app status publisher',
            state: 'unknown',
        });
        this._applyFacet(this._syncRow, {
            summary: 'Desktop status not publishing',
            detail: 'Start berrykeep-config-app to publish merged service status',
            state: 'unknown',
        });
        ...
        this._targetRow.valueLabel.text = 'BerryKeep';
...
export default class BerryKeepStatusExtension extends Extension {
    enable() {
        this._indicator = new BerryKeepIndicator(this);
