...
extension BerryKeepConnectionDraft {
    init(bundleConfiguration: BerryKeepBundleConfiguration) {
        self.init(
    ...
    init(bundleConfiguration: BerryKeepBundleConfiguration, storedState: AppleStoredConnectionState) {
        self.init(
...
typealias BerryKeepDomainRegistrationState = AppleFileProviderDomainRegistrationState

struct BerryKeepRecentAction: Equatable, Identifiable, Sendable {
    let title: String
    let detail: String
    let timestamp: Date
...
struct BerryKeepLibraryBreadcrumb: Equatable, Identifiable, Sendable {
    let path: String
    let label: String
...
enum BerryKeepPreviewPayload: Equatable, Sendable {
    case text(String)
    case image(Data)
    case binary(String)
}

struct BerryKeepFilePreviewResult: Equatable, Sendable {
    let filename: String
    let byteCount: Int
    let payload: BerryKeepPreviewPayload
}

struct BerryKeepConnectionAttemptStatus: Codable, Equatable, Identifiable, Sendable {
...
struct BerryKeepConnectionEndpointStatus: Codable, Equatable, Identifiable, Sendable {
    var pathKind: String
    ...
    var recentAttempts: [BerryKeepConnectionAttemptStatus]
...
struct BerryKeepConnectionDiagnosticsSnapshot: Codable, Equatable, Sendable {
    var connectionName: String?
    var generatedAtUnixMs: UInt64?
    var endpoints: [BerryKeepConnectionEndpointStatus]
    var lastSuccessUnixMs: UInt64?
}

struct BerryKeepWebUIPresentation: Identifiable, Equatable, Sendable {
...
@MainActor
final class BerryKeepBrowserModel: ObservableObject {
    @Published var draft: BerryKeepConnectionDraft {
        didSet {
    ...
    @Published var domainState: BerryKeepDomainRegistrationState = .unchecked
    ...
    @Published var recentActions: [BerryKeepRecentAction] = []
    @Published var lastLibraryRefreshAt: Date?
    @Published var filesSelectionSummary: String?
    @Published var connectionDiagnostics: BerryKeepConnectionDiagnosticsSnapshot?
    ...
    @Published var webUIPresentation: BerryKeepWebUIPresentation?
    ...
    let remoteSession: BerryKeepRemoteSession
    ...
    init(
        ...
        enroller: AppleBootstrapEnroller = BerryKeepRustFFIAdapter(connectionName: "ios app shell"),
        ...
        let bundleConfiguration = BerryKeepBundleConfiguration(bundle: .main)
        let defaults = BerryKeepConnectionDraft(bundleConfiguration: bundleConfiguration)
        ...
        let storedDraft: BerryKeepConnectionDraft? = if let storedData = userDefaults.data(
            forKey: draftStorageKey
        ) {
            try? JSONDecoder().decode(BerryKeepConnectionDraft.self, from: storedData)
        } else {
            nil
        }
        ...
        let initialDraft: BerryKeepConnectionDraft
        if var storedDraft {
            ...
            initialDraft = BerryKeepConnectionDraft(
    ...
    var healthHeadline: String {
        ...
        if isBusy && lastSuccessfulConnectionAt == nil {
            return "Connecting to BerryKeep"
        }
    ...
    func diagnosticLogText() -> String {
        ...
        let rustLog: String
        do {
            rustLog = try BerryKeepRustFFIAdapter(
        ...
        let applicationLog = diagnosticActions
            .map { action in
                "\(formatter.string(from: action.timestamp)) INFO/BerryKeepIosApp: \(action.title) — \(action.detail)"
            }
            .joined(separator: "\n")
    ...
    var breadcrumbs: [BerryKeepLibraryBreadcrumb] {
        ...
        return components.map { component in
            ...
            return BerryKeepLibraryBreadcrumb(path: segments.joined(separator: "/"), label: component)
        }
    }
    ...
    var orderedConnectionEndpoints: [BerryKeepConnectionEndpointStatus] {
    ...
    var recentConnectionAttempts: [BerryKeepConnectionAttemptStatus] {
    ...
    private func applyWebUIState(_ state: AppleWebUiState) -> Bool {
        ...
        case (.running, .webUI, let session?):
            webUIPresentation = BerryKeepWebUIPresentation(session: session)
            galleryMapPresentation = nil
        case (.running, .galleryMap, let session?):
            webUIPresentation = nil
            galleryMapPresentation = BerryKeepWebUIPresentation(session: session)
        default:
            webUIPresentation = nil
            galleryMapPresentation = nil
        }
    ...
    func loadPreview(for item: AppleBridgeItem) async -> BerryKeepFilePreviewResult {
        guard draft.isConfigured else {
            return BerryKeepFilePreviewResult(
    ...
    private func addAction(_ title: String, detail: String) {
        let action = BerryKeepRecentAction(title: title, detail: detail, timestamp: Date())
...
private func clearBerryKeepCachedFiles() {
    ...
    let berrykeepCacheDirectory = cachesDirectory.appendingPathComponent("BerryKeep", isDirectory: true)
    try? fileManager.removeItem(at: berrykeepCacheDirectory)
}
...
final class BerryKeepRemoteSession: @unchecked Sendable {
    private let bridge: AppleCFacadeBridge

    init(ffi: AppleManualCBridgeFFI = BerryKeepRustFFIAdapter(connectionName: "ios app shell")) {
    ...
    func connectionDiagnostics(
        configuration: AppleConnectionConfiguration
    ) throws -> BerryKeepConnectionDiagnosticsSnapshot {
