...
struct BerryKeepGalleryLoadContext: Equatable, Sendable {
    let configuration: AppleConnectionConfiguration
    let query: AppleGalleryQuery
}

@MainActor
final class BerryKeepGalleryModel: ObservableObject {
    @Published private(set) var entries: [AppleStoreIndexEntry] = []
    @Published private(set) var totalCount = 0
    @Published private(set) var isLoading = false
    @Published private(set) var errorMessage: String?

    let imageRepository: BerryKeepGalleryImageRepository

    private let remoteSession: BerryKeepRemoteSession
    private var activeContext: BerryKeepGalleryLoadContext?
    ...
    init(
        remoteSession: BerryKeepRemoteSession,
        imageRepository: BerryKeepGalleryImageRepository
    ) {
    ...
    func reload(
        ...
        let context = BerryKeepGalleryLoadContext(
    ...
    private func loadPage(
        context: BerryKeepGalleryLoadContext,
        generation: UInt64,
        offset: Int
    ) {
...
final class BerryKeepGalleryImageRepository: @unchecked Sendable {
    ...
    private let requestLimiter = BerryKeepGalleryRequestLimiter(limit: 4)
    ...
    private func cacheLookup(
        ...
        guard let generation = cacheContextGate.generation(for: configuration) else {
            throw BerryKeepGalleryImageRepositoryError.staleConnectionContext
        }
...
private actor BerryKeepGalleryRequestLimiter {
...
private enum BerryKeepGalleryImageRepositoryError: LocalizedError {
