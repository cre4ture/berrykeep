...
extension BerryKeepFileProviderService {
    ...
    private func contentSelection(
        for identifier: NSFileProviderItemIdentifier,
        item: AppleBridgeItem
    ) throws -> BerryKeepContentSelection {
        let appleIdentifier = try appleIdentifier(from: identifier)
        if let token = appleIdentifier.originalShareToken {
            let capability: AppleOriginalShareCapability
            do {
            ...
            return BerryKeepContentSelection(
                item: item,
    ...
    private func streamSelection(
        _ selection: BerryKeepContentSelection,
        range: Range<UInt64>,
        to fileURL: URL,
        ...
        while offset < range.upperBound {
            ...
            let length = Int(min(
                UInt64(BerryKeepContentSelection.maximumRangeBytes),
...
private struct BerryKeepContentSelection {
