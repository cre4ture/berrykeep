...
protocol BerryKeepSyncEnvironmentProviding: Sendable {
    func snapshot() -> AppleSyncEnvironmentSnapshot
}

protocol BerryKeepSyncEnvironmentObserving: BerryKeepSyncEnvironmentProviding {
...
final class BerryKeepLiveSyncEnvironment: BerryKeepSyncEnvironmentObserving, @unchecked Sendable {
    static let shared = BerryKeepLiveSyncEnvironment()

    private let monitor = NWPathMonitor()
    private let lock = NSLock()
    private let callbackQueue = DispatchQueue(label: "dev.berrykeep.apple.sync-environment-callbacks")
    ...
    private init() {
        ...
        monitor.start(queue: DispatchQueue(label: "dev.berrykeep.apple.sync-network-path"))
    }
