...
final class BerryKeepFileProviderEnumerator: NSObject, NSFileProviderEnumerator, @unchecked Sendable {
    private let containerIdentifier: NSFileProviderItemIdentifier
    private let service: BerryKeepFileProviderService

    init(containerIdentifier: NSFileProviderItemIdentifier, service: BerryKeepFileProviderService) {
    ...
    func enumerateChanges(for observer: NSFileProviderChangeObserver, from syncAnchor: NSFileProviderSyncAnchor) {
        ...
        DispatchQueue.global(qos: .utility).async {
            let observer = observerBox.value
            do {
                let anchorGeneration = try BerryKeepSyncAnchorCodec.generation(from: syncAnchor)
                guard self.containerIdentifier == .workingSet else {
                    let generation = try self.service.currentChangeGeneration()
                    observer.finishEnumeratingChanges(
                        upTo: BerryKeepSyncAnchorCodec.anchor(for: generation),
                        moreComing: false
                    )
                    return
                }
                ...
                observer.finishEnumeratingChanges(
                    upTo: BerryKeepSyncAnchorCodec.anchor(for: changes.batch.generation),
                    moreComing: false
                )
    ...
    func currentSyncAnchor(completionHandler: @escaping (NSFileProviderSyncAnchor?) -> Void) {
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            completion.value(BerryKeepSyncAnchorCodec.anchor(for: generation))
        }
    }
}
