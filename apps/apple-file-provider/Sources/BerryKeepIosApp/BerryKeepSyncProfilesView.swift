...
struct BerryKeepFilesView: View {
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    if let error = model.syncProfilesErrorMessage {
                        BerryKeepCard(title: "Sync profiles need attention") {
                    ...
                    if model.syncProfiles.isEmpty {
                        ...
                        ForEach(model.syncProfiles) { profile in
                            BerryKeepSyncProfileCard(profile: profile)
                        }
                    }

                    BerryKeepCard(title: "Native Files lifecycle", subtitle: "No foreground timer or WorkManager port is used.") {
        ...
        .sheet(isPresented: $showsFilesPicker) {
            BerryKeepFilesHandoffPicker { url in model.noteFilesSelection(url) }
        }
        .sheet(isPresented: $showsNewProfile) {
            BerryKeepNewSyncProfileSheet()
        }
    }

    private var connectionDiagnostics: some View {
        BerryKeepCard(title: "Connection diagnostics", subtitle: "App-side visibility into the current route.") {
            BerryKeepKeyValueRow(label: "Connection role", value: model.connectionDiagnostics?.connectionName ?? "ios app shell")
            BerryKeepKeyValueRow(label: "Status", value: model.statusText)
            BerryKeepKeyValueRow(label: "Connection target", value: model.draft.normalizedConnectionInput ?? "Not configured")
            BerryKeepKeyValueRow(label: "Last route success", value: syncUnixMillisecondsTimestamp(model.connectionDiagnostics?.lastSuccessUnixMs))
            BerryKeepKeyValueRow(label: "Last library success", value: syncTimestamp(model.lastSuccessfulConnectionAt))
            BerryKeepKeyValueRow(label: "Last root refresh", value: syncTimestamp(model.lastLibraryRefreshAt))
            if let lastErrorMessage = model.lastErrorMessage {
                BerryKeepKeyValueRow(label: "Last error", value: lastErrorMessage)
            }
            HStack {
            ...
            if model.orderedConnectionEndpoints.isEmpty {
                ...
                ForEach(Array(model.orderedConnectionEndpoints.prefix(3))) { endpoint in
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        BerryKeepKeyValueRow(label: "Path", value: endpoint.pathKind)
                        BerryKeepKeyValueRow(label: "Base URL", value: endpoint.requestBaseUrl)
                        BerryKeepKeyValueRow(label: "Failures", value: "\(endpoint.consecutiveFailures) consecutive, \(endpoint.totalFailures) total")
...
private func endpointWasRecentlyUsed(
    _ endpoint: BerryKeepConnectionEndpointStatus,
...
private struct BerryKeepSyncProfileCard: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    ...
    var body: some View {
        ...
        BerryKeepCard(
            ...
            BerryKeepKeyValueRow(label: "Remote scope", value: profile.scopeSummary)
            BerryKeepKeyValueRow(label: "Files domain", value: profile.domainIdentifier)
            BerryKeepKeyValueRow(label: "Connection", value: "Shared enrolled device")
            BerryKeepKeyValueRow(label: "Domain state", value: domainStateLabel(registeredDomain))
            BerryKeepKeyValueRow(label: "Network", value: profileNetworkSummary(profile))
            BerryKeepKeyValueRow(
                label: "Offline files",
...
private struct BerryKeepNewSyncProfileSheet: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
