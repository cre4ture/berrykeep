...
@main
struct BerryKeepIosApp: App {
    @StateObject private var model = BerryKeepBrowserModel()

    var body: some Scene {
        WindowGroup {
            if let session = BerryKeepUiTestWebUiSession.fromLaunchEnvironment() {
                if BerryKeepUiTestWebUiSession.embeddedSurface == .galleryMap {
                    BerryKeepGalleryMapContent(
                        session: session,
                    ...
                    BerryKeepHostedWebView(
                        session: session,
                        accentColorHex: AppleAccentColor.defaultHex
                    )
                }
            } else {
                BerryKeepIosRootView()
                    .environmentObject(model)
                    .task {
                        model.activate()
                    }
                    .tint(Color(berrykeepAccentColorHex: model.themeAccentColorHex))
            }
        }
    }
}

private struct BerryKeepDiagnosticLogDocument: FileDocument {
...
private enum BerryKeepUiTestWebUiSession {
    static var embeddedSurface: BerryKeepEmbeddedSurface? {
        BerryKeepEmbeddedSurface(
            rawValue: ProcessInfo.processInfo.environment["BERRYKEEP_UI_TEST_EMBEDDED_SURFACE"] ?? ""
        )
    }

    static func fromLaunchEnvironment() -> AppleWebUiSession? {
        guard let url = ProcessInfo.processInfo.environment["BERRYKEEP_UI_TEST_WEB_UI_URL"],
...
private struct BerryKeepIosRootView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    @Environment(\.scenePhase) private var scenePhase

    var body: some View {
        Group {
            if model.shouldShowOnboarding {
                BerryKeepOnboardingGateView()
            } else {
                BerryKeepMainShellView()
            }
        }
        .sheet(
            item: Binding(
...
private enum BerryKeepMainShellTab: Hashable {
    case home
    case library
    case galleryMap
    case files
...
private struct BerryKeepMainShellView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    @State private var selectedTab: BerryKeepMainShellTab = .home
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            BerryKeepHomeView()
                ...
                .tag(BerryKeepMainShellTab.home)

            BerryKeepLibraryView()
                .tabItem {
                    Label("Library", systemImage: "books.vertical")
                }
                .tag(BerryKeepMainShellTab.library)

            BerryKeepGalleryMapView()
                .tabItem {
                    Label("Gallery Map", systemImage: "map")
                }
                .tag(BerryKeepMainShellTab.galleryMap)

            BerryKeepFilesView()
                .tabItem {
                    Label("Files", systemImage: "folder.badge.gearshape")
                }
                .tag(BerryKeepMainShellTab.files)

            BerryKeepSettingsView()
                .tabItem {
                    Label("Settings", systemImage: "gearshape")
                }
                .tag(BerryKeepMainShellTab.settings)
        }
...
private enum BerryKeepEmbeddedSurface: String {
    case galleryMap = "gallery_map"
}

private struct BerryKeepGalleryMapView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
...
private struct BerryKeepGalleryMapContent: View {
    ...
    private var galleryMapStartCard: some View {
        ...
        .accessibilityIdentifier("berrykeep-gallery-map-start")
    }
}
...
private func galleryMapWebUiSession(from session: AppleWebUiSession) -> AppleWebUiSession? {
    ...
    queryItems.append(URLQueryItem(name: "embedded", value: BerryKeepEmbeddedSurface.galleryMap.rawValue))
...
private struct BerryKeepOnboardingGateView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    BerryKeepCard(title: "Quick setup", subtitle: "Import the routes and trust material advertised by your cluster.") {
                    ...
                    BerryKeepCard(title: "What happens next", subtitle: model.draft.enrollmentSummary) {
                        BerryKeepKeyValueRow(label: "Connection", value: model.draft.setupSummary)
                        BerryKeepKeyValueRow(label: "Identity", value: model.draft.identitySummary)
                        BerryKeepKeyValueRow(label: "Device", value: model.draft.enrolledDeviceID.nilIfBlank ?? "Not enrolled yet")
                        BerryKeepKeyValueRow(label: "Domain", value: model.draft.domainDisplayName)
                    ...
                    if let lastErrorMessage = model.lastErrorMessage {
                        BerryKeepCard(title: "Last issue", subtitle: "Resolve this before continuing.") {
        ...
        .sheet(isPresented: $showsScanner) {
            BerryKeepScannerSheet { payload in
                model.applyScannedCode(payload)
            }
        }
    }

    private func draftBinding(_ keyPath: WritableKeyPath<BerryKeepConnectionDraft, String>) -> Binding<String> {
        Binding(
...
private struct BerryKeepHomeView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    LazyVGrid(
                        ...
                        BerryKeepMetricTile(label: "Root items", value: "\(model.items.count)")
                        BerryKeepMetricTile(label: "Folders", value: "\(model.rootDirectoryCount)")
                        BerryKeepMetricTile(label: "Files", value: "\(model.rootFileCount)")
                        BerryKeepMetricTile(label: "Last success", value: relativeDate(model.lastSuccessfulConnectionAt))
                    }

                    BerryKeepCard(title: "Connection overview", subtitle: model.draft.enrollmentSummary) {
                        BerryKeepKeyValueRow(label: "Status", value: model.statusText)
                        BerryKeepKeyValueRow(label: "Connection", value: model.draft.normalizedConnectionInput ?? "Not configured")
                        BerryKeepKeyValueRow(label: "Role", value: model.connectionDiagnostics?.connectionName ?? "ios app shell")
                        BerryKeepKeyValueRow(label: "Identity", value: model.draft.identitySummary)
                        if let enrolledDeviceID = model.draft.enrolledDeviceID.nilIfBlank {
                            BerryKeepKeyValueRow(label: "Device ID", value: enrolledDeviceID)
                        }
                        BerryKeepKeyValueRow(label: "Domain", value: model.domainState.title)
                        if let filesSelectionSummary = model.filesSelectionSummary {
                            BerryKeepKeyValueRow(label: "Files handoff", value: filesSelectionSummary)
                        }
                    }

                    BerryKeepCard(title: "Recent activity", subtitle: "The latest app-side actions and checks.") {
            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    BerryKeepTitleLatencyToolbarItem()
                }
            }
        }
        .sheet(isPresented: $showsFilesPicker) {
            BerryKeepFilesHandoffPicker { url in
                model.noteFilesSelection(url)
            }
        }
    }

    private func toneForHome(model: BerryKeepBrowserModel) -> BerryKeepHeroTone {
...
private struct BerryKeepLibraryView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    @State private var selectedFilePath: String?
    @State private var showsFilesPicker = false
    @State private var section: BerryKeepLibrarySection = .browse

    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                Picker("Library section", selection: $section) {
                    Label("Browse", systemImage: "folder").tag(BerryKeepLibrarySection.browse)
                    Label("Photos", systemImage: "photo.on.rectangle").tag(BerryKeepLibrarySection.photos)
                }
                ...
                if section == .browse {
                    ScrollView {
                        VStack(spacing: 18) {
                            BerryKeepCard(title: "Remote browser", subtitle: "Browse the remote library directly from the app.") {
                                BerryKeepKeyValueRow(label: "Current path", value: displayPath(model.currentPath))
                                HStack {
                                    Button("Root") {
                                        model.browse(path: "")
                                    }
                            ...
                            if !model.breadcrumbs.isEmpty {
                                BerryKeepCard(title: "Breadcrumbs") {
                    ...
                    BerryKeepGalleryView(remoteSession: model.remoteSession)
                }
            }
            .background(Color(uiColor: .systemGroupedBackground))
        ...
        .sheet(
            ...
            if let item = model.item(at: selectedFilePath) {
                BerryKeepFileInspectorView(item: item)
            } else {
...
private enum BerryKeepLibrarySection {
    case browse
    case photos
}

struct BerryKeepConnectionPathsView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
...
private struct BerryKeepConnectionOverviewRow: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if expanded {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    BerryKeepKeyValueRow(label: "Direct", value: "\(snapshot.directEndpointCount)")
                    BerryKeepKeyValueRow(label: "Relay", value: "\(snapshot.relayEndpointCount)")
                    BerryKeepKeyValueRow(
                        label: "Evaluated",
...
private struct BerryKeepConnectionPathRow: View {
            ...
            if expanded {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if let targetNodeDetail = endpoint.targetNodeDetail {
                        BerryKeepKeyValueRow(label: "Target server node", value: targetNodeDetail)
                    }
                    ...
                    if let relayUrl = endpoint.lastSuccessfulIrohRelayUrl {
                        BerryKeepKeyValueRow(label: "Last successful Iroh relay", value: relayUrl)
                    }
                    BerryKeepKeyValueRow(label: "Iroh endpoint", value: endpoint.locator)
                    Group {
                        BerryKeepKeyValueRow(label: "EWMA latency", value: milliseconds(endpoint.ewmaLatencyMs))
                        BerryKeepKeyValueRow(
                            label: "EWMA throughput",
                            value: bytesPerSecond(endpoint.ewmaThroughputBytesPerSec)
                        )
                        BerryKeepKeyValueRow(label: "Score", value: String(format: "%.2f", endpoint.score))
                        BerryKeepKeyValueRow(label: "Successes", value: "\(endpoint.totalSuccesses)")
                        BerryKeepKeyValueRow(
                            label: "Failures",
                            value: "\(endpoint.consecutiveFailures) consecutive, \(endpoint.totalFailures) total"
                        )
                        BerryKeepKeyValueRow(label: "Bootstrap rank", value: "\(endpoint.bootstrapRank)")
                        if let holePunchingMode = endpoint.holePunchingMode {
                            BerryKeepKeyValueRow(label: "QUIC path", value: holePunchingMode.capitalized)
                        }
                        BerryKeepKeyValueRow(
                        ...
                        if let lastError = endpoint.lastError {
                            BerryKeepKeyValueRow(label: "Last error", value: lastError)
                        }
                    }
                    .font(.footnote)
                }
...
struct BerryKeepTitleLatencyToolbarItem: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    @State private var diagnosticLogDocument = BerryKeepDiagnosticLogDocument()
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Button {
                diagnosticLogDocument = BerryKeepDiagnosticLogDocument(
...
private struct BerryKeepSettingsView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                if let enrolledDeviceID = model.draft.enrolledDeviceID.nilIfBlank {
                    Section("Device") {
                        BerryKeepInlineNote(text: "Enrolled device: \(enrolledDeviceID)")
                    }
                }
                    ...
                    HStack(spacing: 12) {
                        ForEach(AppleAccentColor.swatches, id: \.self) { swatch in
                            Button {
                                ...
                                Circle()
                                    .fill(Color(berrykeepAccentColorHex: swatch))
                                    .frame(width: 26, height: 26)
                                    .overlay {
    ...
    private var accentColorBinding: Binding<Color> {
        Binding(
            get: { Color(berrykeepAccentColorHex: model.themeAccentColorHex) },
            set: { color in
                guard let hex = berrykeepAccentColorHex(from: color) else {
                    return
                }
...
private struct BerryKeepFileInspectorView: View {
    @EnvironmentObject private var model: BerryKeepBrowserModel
    @Environment(\.dismiss) private var dismiss

    let item: AppleBridgeItem

    @State private var preview: BerryKeepFilePreviewResult?
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 18) {
                    BerryKeepCard(title: item.displayName, subtitle: item.kind == .directory ? "Directory" : "File") {
                        BerryKeepKeyValueRow(label: "Path", value: item.path)
                        BerryKeepKeyValueRow(label: "Identifier", value: item.identifier.serialized)
                        BerryKeepKeyValueRow(label: "Revision", value: item.revisionHint ?? "Unavailable")
                        BerryKeepKeyValueRow(label: "Size", value: byteCount(item.sizeBytes))
                        BerryKeepKeyValueRow(label: "Modified", value: unixTimestamp(item.modifiedAtUnix))
                    }

                    BerryKeepCard(title: "Preview", subtitle: "A pragmatic view into the selected file.") {
                        if isLoading {
                            ProgressView()
...
struct BerryKeepCard<Content: View>: View {
    let title: String
    var subtitle: String?
...
struct BerryKeepHeroCard<Content: View>: View {
    let title: String
    let message: String
    var tone: BerryKeepHeroTone = .neutral
    @ViewBuilder var content: Content

    init(
        title: String,
        body: String,
        tone: BerryKeepHeroTone = .neutral,
...
private struct BerryKeepMetricTile: View {
    let label: String
    let value: String
...
struct BerryKeepBrowserRow: View {
    let item: AppleBridgeItem
...
private extension Color {
    init(berrykeepAccentColorHex: String) {
        let normalized = AppleAccentColor.normalizedHex(berrykeepAccentColorHex)
...
private func berrykeepAccentColorHex(from color: Color) -> String? {
    var red: CGFloat = 0
    var green: CGFloat = 0
    var blue: CGFloat = 0
    var alpha: CGFloat = 0
...
struct BerryKeepKeyValueRow: View {
    let label: String
    let value: String
...
private struct BerryKeepMultilineField: View {
    let title: String
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            BerryKeepMultilineEditor(title: title, text: text, prompt: prompt)
        }
    }
}

private struct BerryKeepMultilineEditor: View {
    let title: String
    let text: Binding<String>
    let prompt: String
...
private struct BerryKeepInlineNote: View {
    let text: String
...
struct BerryKeepFilesHandoffPicker: UIViewControllerRepresentable {
    let onPick: (URL) -> Void
...
private struct BerryKeepHostedWebView: UIViewControllerRepresentable {
    private static let originalShareHandlerName = "BerryKeepIosShare"

    let session: AppleWebUiSession
    let accentColorHex: String

    func makeCoordinator() -> Coordinator {
        Coordinator(origin: berrykeepIosEmbeddedWebURL(session.url, accentColorHex: accentColorHex))
    }
    ...
    func makeUIViewController(context: Context) -> UIViewController {
        ...
        webView.accessibilityIdentifier = "berrykeep-hosted-web-ui"
        webView.allowsLinkPreview = false
        webView.navigationDelegate = context.coordinator
        context.coordinator.webView = webView

        var request = URLRequest(
            url: berrykeepIosEmbeddedWebURL(session.url, accentColorHex: accentColorHex)
        )
        request.setValue(session.authorization, forHTTPHeaderField: "X-BerryKeep-Web-Ui-Session")
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKDownloadDelegate, UIDocumentPickerDelegate, WKScriptMessageHandlerWithReply {
        ...
        private let originalShareCoordinator: BerryKeepOriginalShareCoordinator?
        ...
        init(origin: URL) {
            self.origin = origin
            do {
                originalShareCoordinator = try BerryKeepOriginalShareCoordinator()
        ...
        func download(
            ...
            let directory = FileManager.default.temporaryDirectory
                .appendingPathComponent("berrykeep-web-downloads", isDirectory: true)
...
private struct BerryKeepScannerSheet: View {
    @Environment(\.dismiss) private var dismiss
...
#if canImport(VisionKit)
@available(iOS 16.0, *)
private struct BerryKeepLiveScanner: UIViewControllerRepresentable {
    let onScan: (String) -> Void
...
private func timestamp(_ date: Date?) -> String {
    ...
    return BerryKeepDateFormatters.timestamp.string(from: date)
}
...
private func relativeDate(_ date: Date?) -> String {
    ...
    return BerryKeepDateFormatters.relative.localizedString(for: date, relativeTo: Date())
}
...
private func unixTimestamp(_ timestamp: Int64?) -> String {
    ...
    return BerryKeepDateFormatters.timestamp.string(from: Date(timeIntervalSince1970: TimeInterval(timestamp)))
}
...
private func unixMillisecondsTimestamp(_ timestamp: UInt64?) -> String {
    ...
    return BerryKeepDateFormatters.timestamp.string(
...
private enum BerryKeepDateFormatters {
