...
final class GalleryMapFullscreenUiTests: XCTestCase {
    ...
    func testFullscreenGalleryMapKeepsTheDirectEmbeddingVisible() {
        ...
        let webView = app.webViews["berrykeep-hosted-web-ui"]
    ...
    private func launchApp(
        ...
        app.launchEnvironment["BERRYKEEP_UI_TEST_WEB_UI_URL"] = webUIURL ?? galleryRuntimeURL
        if let embeddedSurface {
            app.launchEnvironment["BERRYKEEP_UI_TEST_EMBEDDED_SURFACE"] = embeddedSurface
        }
        app.launch()
        return app
    }
    ...
    private var galleryRuntimeURL: String {
        ProcessInfo.processInfo.environment["BERRYKEEP_GALLERY_RUNTIME_URL"]
