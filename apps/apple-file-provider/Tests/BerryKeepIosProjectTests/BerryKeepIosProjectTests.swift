...
final class BerryKeepIosProjectTests: XCTestCase {
    func testRevisionConflictMapsToCannotSynchronizeWithCurrentRevision() {
        let error = berrykeepRevisionConflictError(
        ...
        XCTAssertEqual(error.userInfo["BerryKeepCurrentRevision"] as? String, "version-2")
    }
    ...
    func testScannedConnectionPayloadExtractsCustomSchemeBootstrap() {
        let scanned = "berrykeep://import?payload=%7B%22cluster%22%3Atrue%7D"

        XCTAssertEqual(
            BerryKeepConnectionDraft.scannedConnectionPayload(from: scanned),
            "{\"cluster\":true}"
        )
    }
    ...
    func testEnrollmentSummaryReflectsIdentityAndBootstrap() {
        let empty = BerryKeepConnectionDraft()
        let bootstrapAndIdentity = BerryKeepConnectionDraft(
    ...
    func testBootstrapWithoutIdentityRequiresEnrollment() {
        let needsEnrollment = BerryKeepConnectionDraft(
            bootstrapInput: "{\"cluster\":true}"
        )
        let ready = BerryKeepConnectionDraft(
            bootstrapInput: "{\"cluster\":true}",
            clientIdentityJSON: "{\"device_id\":\"ios-demo\"}"
        )
    ...
    func testPersistedBootstrapClaimStillRequiresEnrollmentWhenIdentityExists() {
        ...
        XCTAssertTrue(BerryKeepConnectionDraft.looksLikeBootstrapClaim(compactClaim))
