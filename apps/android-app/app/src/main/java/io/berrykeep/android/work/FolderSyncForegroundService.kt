package io.berrykeep.android.work
...
import io.berrykeep.android.data.FolderSyncStorageDiagnosticsHelper
...
class FolderSyncForegroundService : Service() {
    ...
    private suspend fun reconcileProfilesLocked(): Boolean {
        return withContext(Dispatchers.IO) {
            val deviceAuth = BerryKeepPreferences.getDeviceAuthState(applicationContext)
            ...
            val profiles = BerryKeepPreferences
    ...
    private suspend fun applyDesiredState(
        ...
        desiredProfiles: List<io.berrykeep.android.data.FolderSyncConfig>,
    ...
    private suspend fun reconcileRequestLocked(
        ...
        return try {
            ...
            updateNotification("BerryKeep sync paused", message)
            false
        } catch (error: Exception) {
    ...
    private fun buildDesiredSignature(
        ...
        serverCaPem: String?,
        clientIdentityJson: String?,
        profiles: List<io.berrykeep.android.data.FolderSyncConfig>,
    ): String {
        return buildString {
    ...
    companion object {
        ...
        private const val CHANNEL_ID = "berrykeep-folder-sync"
        private const val NOTIFICATION_ID = 4001
        private const val ACTION_REFRESH = "io.berrykeep.android.action.FOLDER_SYNC_REFRESH"
        private const val ACTION_RESTORE = "io.berrykeep.android.action.FOLDER_SYNC_RESTORE"
        private const val ACTION_SYNC_NOW = "io.berrykeep.android.action.FOLDER_SYNC_NOW"
        private const val ACTION_BACKOFF_TIMER = "io.berrykeep.android.action.FOLDER_SYNC_BACKOFF_TIMER"
        private const val ACTION_APP_FOREGROUNDED = "io.berrykeep.android.action.FOLDER_SYNC_APP_FOREGROUNDED"
        private const val ACTION_LOCAL_FOLDER_CHANGED = "io.berrykeep.android.action.FOLDER_SYNC_LOCAL_FOLDER_CHANGED"
        private const val ACTION_STOP = "io.berrykeep.android.action.FOLDER_SYNC_STOP"
        ...
        fun appForegrounded(context: Context) {
            ...
            triggerScope.launch {
                if (BerryKeepPreferences.getFolderSyncConfigs(appContext).none { profile -> profile.enabled }) {
                    return@launch
                }
        ...
        fun localFolderChanged(context: Context, treeUriString: String) {
            ...
            triggerScope.launch {
                ...
                val hasMatchingEnabledProfile = BerryKeepPreferences
