...
import io.berrykeep.android.api.StoreIndexResponse
import io.berrykeep.android.api.StoreIndexSortOrder
...
import io.berrykeep.android.data.FolderSyncNetworkPolicy
import io.berrykeep.android.data.GlobalFolderSyncStatus
import io.berrykeep.android.data.FolderSyncServiceStatus
import io.berrykeep.android.data.mergeGlobalFolderSyncStatus
import io.berrykeep.android.ui.screens.ThumbnailBitmapCache
import io.berrykeep.android.data.BerryKeepPreferences
import io.berrykeep.android.data.BerryKeepRepository
import io.berrykeep.android.data.RustPreferencesBridge
import io.berrykeep.android.data.TitleLatencyMonitorSettings
import io.berrykeep.android.data.TitleLatencyProbeStatus
import io.berrykeep.android.data.parseAllowedWifiSsidsInput
...
class MainViewModel(
            ...
            val persistedResult = withContext(Dispatchers.IO) {
                runCatching {
                    PersistedMainUiState(
                        syncProfiles = BerryKeepPreferences.getFolderSyncConfigs(getApplication()),
                        deviceAuthResult = runCatching {
                            BerryKeepPreferences.getDeviceAuthState(getApplication())
                        },
                        appConnectionStatus = RustPreferencesBridge.appConnectionStatus()
                            .filterNotNull()
                            .first(),
                        galleryViewMode = BerryKeepPreferences.getGalleryViewMode(getApplication()),
                        titleLatencyMonitorSettings =
                            BerryKeepPreferences.getTitleLatencyMonitorSettings(getApplication()),
                        themeAccentColorHex = BerryKeepPreferences.getThemeAccentColor(getApplication()),
                    )
                }
            }
    ...
    private fun persistFolderSyncProfiles(profiles: List<FolderSyncConfig>) {
        persistPreference {
            BerryKeepPreferences.setFolderSyncConfigs(getApplication(), profiles)
    ...
    fun updateThemeAccentColor(value: String) {
        val normalized = normalizeBerryKeepAccentColorHex(value) ?: return
        uiState.value = uiState.value.copy(themeAccentColorHex = normalized)
        persistPreference {
            BerryKeepPreferences.setThemeAccentColor(getApplication(), normalized)
        }
    }
    ...
    fun updateNodeConnectionPriorityOverride(nodeId: String, priority: Int?) {
        ...
        runCatching { current.withNodePriorityOverride(nodeId, priority) }
            .onSuccess { updated ->
                BerryKeepPreferences.setDeviceAuthState(getApplication(), updated)
    ...
    private fun updateTitleLatencyMonitorSettings(settings: TitleLatencyMonitorSettings) {
        ...
        persistPreference {
            BerryKeepPreferences.setTitleLatencyMonitorSettings(getApplication(), normalized)
        }
        titleLatencyBackgroundGrace.cancel()
        titleLatencyBackgroundStopJob?.cancel()
        titleLatencyBackgroundStopJob = null
    ...
    fun updateGalleryViewMode(mode: GalleryViewMode) {
        ...
        persistPreference {
            BerryKeepPreferences.setGalleryViewMode(getApplication(), mode)
        }
        refreshGallery()
    }
