...
fn object_url_builder_escapes_segments() {
    let client = BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/");
    let url = client
...
fn client_clones_keep_the_same_connection_runtime_id() {
    ...
    let rebuilt = BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/");
...
async fn background_probe_refresh_publishes_maintenance_diagnostics() {
    ...
    let client = BerryKeepClient::from_direct_base_url(endpoint)
...
fn route_reconciliation_preserves_static_state_and_retires_dynamic_routes() {
    let static_client = BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/");
    let original_endpoint = static_client
        .transport_router
        .endpoint(0)
        .expect("static endpoint should exist");
    let dynamic_quic = BerryKeepClient::from_direct_quic_candidate_with_target_node_id(
        ConnectionCandidate {
            kind: CandidateKind::DirectQuic,
            endpoint: "iroh://dynamic-node-key".to_string(),
            rtt_ms: None,
    ...
    let refreshed = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/"),
        dynamic_quic,
    ])
    ...
    let static_only = BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/");
...
fn reintroduced_failed_route_retains_backoff_without_reusing_transport() {
    ...
    let static_route = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/");
    let dynamic_route = || {
        BerryKeepClient::from_direct_quic_candidate_with_target_node_id(
            ConnectionCandidate {
                kind: CandidateKind::DirectQuic,
    ...
    let client = BerryKeepClient::combine(vec![static_route(), dynamic_route()])
    ...
    let replacement = BerryKeepClient::combine(vec![static_route(), dynamic_route()])
...
fn mobile_background_policy_claims_one_candidate_per_batch() {
    let client = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/"),
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18081/"),
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18082/"),
    ])
...
fn stable_route_plan_survives_registry_reordering() {
    let route_a = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18100/");
    let route_b = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18101/");
    let client =
        BerryKeepClient::combine(vec![route_a(), route_b()]).expect("routes should combine");
    let planned = client.transport_router.foreground_route_ids();
    let planned_primary = planned[0].clone();

    let reordered =
        BerryKeepClient::combine(vec![route_b(), route_a()]).expect("routes should reorder");
...
fn preflight_failure_is_recorded_against_captured_route_after_reordering() {
    let route_a = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18105/");
    let route_b = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18106/");
    let client =
        BerryKeepClient::combine(vec![route_a(), route_b()]).expect("routes should combine");
    let mut execution =
...
fn route_use_is_recorded_against_captured_route_after_reordering() {
    let route_a = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18107/");
    let route_b = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18108/");
    let client =
        BerryKeepClient::combine(vec![route_a(), route_b()]).expect("routes should combine");
    let mut execution =
        ForegroundRequestExecutor::new(&client, client.transport_router.foreground_route_ids());
...
fn concurrent_plan_avoids_route_after_another_request_reports_failure() {
    let client = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18110/"),
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18111/"),
    ])
    .expect("routes should combine");
...
fn availability_target_stops_warming_additional_probation_routes() {
    let client = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18120/"),
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18121/"),
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18122/"),
        BerryKeepClient::from_direct_base_url("http://127.0.0.1:18123/"),
    ])
    .expect("routes should combine")
...
fn completed_probe_does_not_update_replaced_endpoint_with_same_route_key() {
    let static_route = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/");
    let dynamic_route = || BerryKeepClient::from_direct_base_url("http://127.0.0.1:18081/");
    let client = BerryKeepClient::combine(vec![static_route(), dynamic_route()])
        .expect("initial routes should combine");
...
fn route_reconciliation_updates_shared_request_identity() {
    ...
    let refreshed = BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080/")
...
fn place_downloaded_file_creates_missing_target_directory() {
    ...
    let root = std::env::temp_dir().join(format!(
        "berrykeep-place-downloaded-file-test-{}-{}",
        std::process::id(),
        nonce
    ));
...
async fn expected_revision_is_sent_separately_for_put_delete_and_recursive_delete() {
    ...
    let client = BerryKeepClient::from_direct_base_url(format!("http://{addr}"));
    client
...
async fn untracked_relative_path_requests_keep_server_failures_out_of_route_diagnostics() {
    ...
    let unavailable_client =
        BerryKeepClient::from_direct_base_url(format!("http://{unavailable_address}"));
    let refreshes = Arc::new(AtomicUsize::new(0));
...
fn direct_http_test_client_for_node(base_url: String, node_id: NodeId) -> BerryKeepClient {
    BerryKeepClient::from_direct_http_client_with_target_node_id_and_ca_pem(
        base_url,
        HttpClient::new(),
        Some(node_id),
...
impl RelayTestSecurity {
    ...
    fn for_cluster(cluster_id: uuid::Uuid, target_node_id: NodeId) -> Self {
        ...
        ca_params
            .distinguished_name
            .push(DnType::CommonName, "berrykeep-client-relay-test-ca");
                    ...
                    write_buffered_transport_response(
                        ...
                        &MultiplexBufferedTransportResponse {
                            ...
                            headers: vec![
                                ...
                                TransportHeader {
                                    name: "x-berrykeep-object-size".to_string(),
                                    value: payload.len().to_string(),
                                },
                            ],
                            body: Vec::new(),
...
fn direct_transport_test_client(
    ...
    BerryKeepClient::from_direct_base_url(state.public_url.clone()).with_client_identity(identity)
}
...
fn relay_test_client(state: &RelayTestState, identity: ClientIdentityMaterial) -> BerryKeepClient {
...
async fn relay_transport_fails_closed_when_source_certificate_identity_mismatches_ticket() {
    ...
    .expect("rendezvous client should build");
    let client = BerryKeepClient::with_relay_transport(
        "https://relay.invalid/",
        rendezvous,
...
async fn upload_session_affinity_survives_route_reconciliation_during_start() {
    ...
    let reordered_routes = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_http_client_with_target_node_id_and_ca_pem(
            node_b_url,
            HttpClient::new(),
            Some(node_b),
...
async fn put_large_aware_skips_chunks_already_available_on_server() {
    ...
    .await;
    let client = BerryKeepClient::from_direct_http_client(base_url, HttpClient::new());
...
async fn direct_quic_continues_after_iroh_relay_ticket_timeout() {
    ...
    let test_result = async {
        ...
        .expect("rendezvous client should build");
        let client = BerryKeepClient::from_direct_quic_candidate_with_rendezvous(
...
async fn rendezvous_backpressure_stops_direct_quic_route_fanout() {
    ...
    let direct_quic_route = |target_node_id| {
        BerryKeepClient::from_direct_quic_candidate_with_rendezvous(
            ConnectionCandidate {
                kind: CandidateKind::DirectQuic,
...
async fn combined_direct_transports_fail_over_to_second_endpoint() {
    ...
    let failing = BerryKeepClient::from_direct_base_url("http://127.0.0.1:9")
        .with_client_identity(identity.clone());
    ...
    let _observer_reset = ConnectionDiagnosticsObserverReset;
    let client = BerryKeepClient::combine(vec![failing, healthy])
        .expect("combined direct client should build")
        .with_connection_name(connection_name);
...
async fn failed_routed_operation_publishes_one_terminal_failure() {
    ...
    let _observer_reset = ConnectionDiagnosticsObserverReset;
    let client =
        BerryKeepClient::from_direct_base_url(endpoint).with_connection_name(connection_name);
...
async fn single_direct_buffered_request_enforces_total_deadline() {
    ...
    let test_result = async {
        ...
        .expect("identity should generate");
        identity.credential_pem = Some("issued-credential".to_string());
        let client = BerryKeepClient::from_direct_base_url(direct_state.public_url.clone())
            .with_client_identity(identity);
        client
            .get_json_path("/cluster/status")
            .await
...
async fn direct_route_stall_falls_back_to_relay_after_warm_session_timeout() {
    ...
    let test_result = async {
        ...
        let direct = BerryKeepClient::from_direct_base_url(direct_state.public_url.clone())
            .with_client_identity(identity.clone());
        let relay = relay_test_client(&relay_state, identity);
        let client = BerryKeepClient::combine(vec![direct, relay])
...
async fn buffered_foreground_request_uses_best_scored_validated_route() {
    ...
    let client = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_base_url(slower_url),
        BerryKeepClient::from_direct_base_url(challenger_url),
    ])
    .expect("combined direct client should build");
...
fn probation_is_transport_agnostic_and_clears_after_successful_probe() {
    ...
    let direct = BerryKeepClient::from_direct_base_url("http://127.0.0.1:18080")
        .with_client_identity(direct_identity.clone());
    let relay_active = BerryKeepClient::combine(vec![relay, direct])
    ...
    let direct = BerryKeepClient::from_direct_base_url("http://127.0.0.1:28080")
    ...
    let direct_active = BerryKeepClient::combine(vec![direct, relay])
        .expect("combined direct+relay client should build");
...
async fn background_probe_reprioritizes_recovered_direct_endpoint() {
    ...
    let primary = BerryKeepClient::from_direct_base_url(primary_url.clone());
    let fallback = BerryKeepClient::from_direct_base_url(fallback_url.clone());
    let client = BerryKeepClient::combine(vec![primary, fallback])
        .expect("combined direct client should build");
...
async fn refresh_connection_route_snapshot_times_out_stalled_probe() {
    ...
    let client = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_base_url(stalled_url.clone()),
        BerryKeepClient::from_direct_base_url(healthy_url.clone()),
    ])
    .expect("combined direct client should build");
...
async fn due_route_refresh_reprobes_only_stale_non_preferred_endpoint_with_warm_samples() {
    ...
    let client = BerryKeepClient::combine(vec![
        BerryKeepClient::from_direct_base_url(preferred_url),
        BerryKeepClient::from_direct_base_url(inactive_url),
    ])
    .expect("combined direct client should build");
...
async fn direct_transport_head_error_includes_endpoint_and_target_node_id() {
    ...
    identity.credential_pem = Some("issued-credential".to_string());
    let target_node_id = NodeId::new_v4();
    let client = BerryKeepClient::from_direct_http_client_with_target_node_id_and_ca_pem(
...
async fn direct_transport_keeps_small_rpcs_responsive_during_streamed_downloads() {
    ...
    .expect("identity should generate");
    identity.credential_pem = Some("issued-credential".to_string());
    let client = BerryKeepClient::from_direct_base_url(base_url).with_client_identity(identity);
...
fn blocking_downloads_handle_concurrent_range_and_staged_requests() {
    fn build_range_response(
        ...
        Response::builder()
            .status(status)
            .header("x-berrykeep-object-size", payload.len().to_string())
...
fn assert_concurrent_staged_downloads(client: &BerryKeepClient, payload: &Arc<Vec<u8>>) {
    let staging_root = std::env::temp_dir().join(format!(
        "berrykeep-concurrent-staged-download-{}",
        Uuid::new_v4()
    ));
