...
focal  arm64  packages/focal/ironmesh-server-node_1.1.0-1_arm64.deb
trixie arm64  packages/trixie/ironmesh-server-node_1.1.0-1_arm64.deb
```
...
`Packages` index. It keeps previous `ironmesh-server-node` versions alongside
the new one, so an unchanged map-tools package with an exact Server Node
dependency remains installable. This preserves desktop packages in their
original suite without allowing Focal packages to appear in Trixie on later
publishes.
