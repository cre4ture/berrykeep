...
- Directionally, the normal UX should be:
  ...
  - first node becomes the initial managed secure rendezvous host automatically,
...
- First implementation slice now in place:
  ...
  - the runtime admin UI/API can now export and import one combined managed control-plane promotion package that moves both signer and embedded rendezvous together to a promoted node, with restart-required activation semantics after import,
...
6. Deliver global rendezvous tenancy for Option 1. Status: implemented by this
   branch, pending review and merge. Phase 1 relay security is on `main`, and
   Phase 2 adds the persistent P2-C registry/dynamic verifier, the versioned
   global registration and operator API, explicit server-node opt-in and
   auto-registration, and two-cluster/restart end-to-end coverage. The feature
   is available on `main` only after this branch merges.
...
- [x] Replace `NodeDescriptor` with a record that separates stable identity from current reachability.
- [x] Keep labels, capacity, and status.
- [x] Replace raw `public_url` / `internal_url` assumptions with a reachability structure that can represent:
...
The following names and semantics are the implemented Phase-2 configuration
contract. They are available on this branch and become available on `main` on
merge.
