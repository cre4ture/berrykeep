...
| Windows sync-root bootstrap filename | `connection-bootstrap.json` under `%LOCALAPPDATA%\Ironmesh\sync-roots\...` | Hidden `.ironmesh-connection.json` paths still exist as legacy/internal connection-config references | `crates/adapter-windows-cfapi/src/connection_config.rs` | Any remaining hidden bootstrap-file discovery is removed or migrated to the current persisted sync-root state family |
| Windows client identity filename discovery | `client-identity.json` under `%LOCALAPPDATA%\Ironmesh\sync-roots\...` or sibling `*.client-identity.json` next to a bootstrap file | Hidden `.ironmesh-client-identity.json` paths still count as internal relative-path matches for compatibility | `crates/adapter-windows-cfapi/src/auth.rs` | Hidden client-identity discovery names are removed and only the documented current names remain |
...
| Folder-agent profile directory fingerprint | `profiles/<blake3(identity-root, scope-prefix, connection-target)>/` under `${XDG_STATE_HOME:-$HOME/.local/state}/ironmesh/folder-agent/` | Legacy `DefaultHasher` profile directories and stored SQLite `scope_fingerprint` values migrate to the stable digest path on open | `crates/sync-agent-core/src/folder_agent_state.rs`, `crates/sync-agent-core/src/folder_agent_modification_log.rs` | The pre-release compatibility window for legacy profile directories closes |
| Store-index directory projection | `view=children` omits the queried directory marker before filtering, sorting, and pagination | A node that rejects the new projection is retried with its established unpaged `view=tree` response, then projected locally before recreating the requested offset page. The rejected capability and at most one complete legacy response expire after five minutes; that response is reused only after a one-entry tree probe confirms its consistency token, so page navigation avoids repeat full-index transfers without serving stale live data or retaining a full tree indefinitely. | `crates/client-sdk/src/ironmesh_client.rs`, `web/packages/api/src/server-admin/client.ts` | The minimum supported server-node version implements `view=children` |
