...
| Store-index directory projection | `view=children` omits the queried directory marker before filtering, sorting, and pagination | A node that rejects the new projection is retried with its established unpaged `view=tree` response, then projected locally before recreating the requested offset page. The rejected capability is cached for five minutes, but legacy tree data is fetched again for every request because its process-local consistency token is not safe across routes or restarts. | `crates/client-sdk/src/ironmesh_client.rs`, `web/packages/api/src/server-admin/client.ts` | The minimum supported server-node version implements `view=children` |
