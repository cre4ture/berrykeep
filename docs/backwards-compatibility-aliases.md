...
| Store-index directory projection | `view=children` omits the queried directory marker before filtering, sorting, and pagination | A node that rejects the new projection is retried with its established unpaged `view=tree` response, then projected locally before recreating the requested offset page. The rejected capability and at most one complete legacy response expire after five minutes; that response is reused only after a one-entry tree probe confirms its consistency token. Captured-media queries do not cache a tree because their paginated probe uses a different gallery-token namespace. | `crates/client-sdk/src/ironmesh_client.rs`, `web/packages/api/src/server-admin/client.ts` | The minimum supported server-node version implements `view=children` |
