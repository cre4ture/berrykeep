...
| Store-index directory projection | `view=children` omits the queried directory marker before filtering, sorting, and pagination | A node that rejects the new projection is retried with its established unpaged `view=tree` response, then projected locally before recreating the requested offset page; the rejected capability is cached with a five-minute reprobe, but no index data is cached | `crates/client-sdk/src/ironmesh_client.rs`, `web/packages/api/src/server-admin/client.ts` | The minimum supported server-node version implements `view=children` |
