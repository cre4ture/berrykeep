...
Die BerryKeep-Server-Admin-Oberfläche bleibt für BerryKeep-Konfiguration,
Status und Datenbetrieb zuständig. Privilegierte Host-Aufgaben werden nicht
in BerryKeep eingebaut, sondern durch Cockpit als getrennte
Host-Administrationsoberfläche erledigt:

- Neustart des von der Installation verwalteten BerryKeep-Service,
- Betriebssystem- und Paket-Updates,
- Neustart oder Herunterfahren des Hosts.

Das passt besonders gut zu Hosts, auf denen der BerryKeep Server Node der
...
Cockpit wird nicht eingebettet, nicht per Reverse Proxy durch BerryKeep
weitergereicht und erhält kein Single Sign-on oder Zugangstoken von BerryKeep.
Damit bleiben die BerryKeep-Admin-Anmeldung und die Host-Root-Verwaltung
...
als **optional** markiert und blockiert keine BerryKeep-Funktion.

Die Detailkarte macht deutlich, dass Cockpit für Service-Neustarts, Updates
und Host-Reboots verwendet werden kann, BerryKeep diese Aktionen jedoch weder
...
1. **Orientierung im bestehenden Admin-UI**
   ...
   - Optionaler Status und Installationshinweis ohne Auswirkung auf
     Funktionsprüfungen.
2. **Konfigurationsänderung in BerryKeep**
...
benötigt keinen privilegierten BerryKeep-Endpunkt.
