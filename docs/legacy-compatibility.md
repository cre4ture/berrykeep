...
| `ironmesh-server-node`, `io.ironmesh.server-node`, `_ironmesh`, and the former macOS service data, configuration, and log paths | `berrykeep-server-node`, `io.berrykeep.server-node`, `_berrykeep`, and BerryKeep macOS service paths | The canonical macOS binary is installed normally and the former executable name is a symlink. The installer stops the former daemon, moves mutable state into the canonical layout when no canonical state exists, and assigns migrated state to the canonical service account. |
...
| `x-ironmesh-*` signed-request, admin-token, and ingestion-token headers, plus the v1 request-signing context | `x-berrykeep-*` signed-request, admin-token, and ingestion-token headers, plus the BerryKeep v1 request-signing context | New clients emit canonical headers and signatures; upgraded servers accept former headers and verify either signing context. Upgrade clients before relying on a canonical-only server. |
