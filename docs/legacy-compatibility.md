...
| `x-ironmesh-*` signed-request headers and the v1 request-signing context | `x-berrykeep-*` signed-request headers and the BerryKeep v1 request-signing context | New clients emit canonical headers and signatures; upgraded servers accept former headers and verify either signing context so clients and nodes can be upgraded independently. |
