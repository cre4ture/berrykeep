...
| `x-ironmesh-*` signed-request and admin-token headers, plus the v1 request-signing context | `x-berrykeep-*` signed-request and admin-token headers, plus the BerryKeep v1 request-signing context | New clients emit canonical headers and signatures; upgraded servers accept former headers and verify either signing context. Upgrade clients before relying on a canonical-only server. |
| `.ironmesh*` internal sync artifacts, conflict-copy roots, Linux FUSE client-rights and download roots, folder-agent and Android folder-sync state roots and profile fingerprints, and `user.ironmesh.*` Linux FUSE xattrs | BerryKeep-named internal artifacts, conflict-copy root, state roots, profile fingerprints, and xattrs | Rust and Android SAF sync scanning and cleanup ignore former artifacts, the folder agent, Linux FUSE adapter, and Android app move recognized state to canonical storage, conflict resolution also reads former copies, and Linux FUSE accepts former xattr reads while listing canonical names. |
