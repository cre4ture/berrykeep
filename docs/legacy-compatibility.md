...
| `.ironmesh*` internal sync artifacts, conflict-copy roots, folder-agent state roots and profile fingerprints, and `user.ironmesh.*` Linux FUSE xattrs | BerryKeep-named internal artifacts, conflict-copy root, state root, profile fingerprints, and xattrs | Rust and Android SAF sync scanning and cleanup ignore former artifacts, the folder agent moves recognized state and profiles to canonical storage, conflict resolution also reads former copies, and Linux FUSE accepts former xattr reads while listing canonical names. |
