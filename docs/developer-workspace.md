...
This document contains the developer-focused material that used to live in the
main README.
...
The Rust `server-node` build triggers `pnpm build` inside `web/`, so the frontend dependencies must be installed in [`web/package.json`](../web/package.json) first. If you accidentally run `pnpm` or `npm` from the repo root, you'll create an unused top-level `node_modules/` that this workspace does not use.
...
- Cross-platform filesystem integration strategy, requirements, and phased plan are documented in [cross-platform-filesystem-integration-strategy.md](cross-platform-filesystem-integration-strategy.md).
...
- Cross-platform filesystem implementation status and platform notes live in [cross-platform-filesystem-integration-strategy.md](cross-platform-filesystem-integration-strategy.md).
- Short coding-session bootstrap context lives in [agent-context.md](agent-context.md).

## CI operations

- CI branch-protection alignment and nightly-lane triage steps are documented in [ci-runbook.md](ci-runbook.md).
