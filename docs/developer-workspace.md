...
- `ironmesh-rendezvous-service` supported runtime envs: `IRONMESH_RENDEZVOUS_BIND`, `IRONMESH_RENDEZVOUS_PUBLIC_URL`, `IRONMESH_RELAY_PUBLIC_URLS`, `IRONMESH_RENDEZVOUS_CLIENT_CA_CERT`, `IRONMESH_RENDEZVOUS_TLS_CERT`, `IRONMESH_RENDEZVOUS_TLS_KEY`, `IRONMESH_RENDEZVOUS_FAILOVER_PACKAGE`, and `IRONMESH_RENDEZVOUS_FAILOVER_PASSPHRASE`. The optional embedded iroh relay uses `IRONMESH_IROH_RELAY_BIND`, `IRONMESH_IROH_RELAY_PUBLIC_URLS`, `IRONMESH_IROH_RELAY_AUTH_TOKEN`, `IRONMESH_IROH_RELAY_CLIENT_RX_BYTES_PER_SECOND`, and `IRONMESH_IROH_RELAY_CLIENT_RX_MAX_BURST_BYTES`; see [Embedded Iroh Relay Operations](iroh-relay-companion-operations.md). Standalone failover startup now expects `--bind-addr`, and new failover exports embed the rendezvous client CA so `IRONMESH_RENDEZVOUS_CLIENT_CA_CERT` is only needed for file-based TLS or legacy failover packages.
