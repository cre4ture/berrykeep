...
- Automated gate evidence:
   ...
   - `justfile` now mirrors the stable CI split correctly: stable unit tests use `cargo test --workspace` without the old redundant `--exclude system-tests` warning, `ci-stable` includes `fmt-check`, and `ci-required` provides one local entry point for the required gate set. That `unit-tests` lane now also picks up the Linux-only packaged config-app handoff integration test in `apps/config-app/tests/package_handoff.rs` automatically on `ubuntu-latest`.
