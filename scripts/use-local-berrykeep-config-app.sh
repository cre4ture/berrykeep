...
PROFILE="${BERRYKEEP_LOCAL_PROFILE:-${DEFAULT_PROFILE}}"
SKIP_BUILD=0
LOCAL_BINARY=""

usage() {
  cat <<'EOF'
Usage: scripts/use-local-berrykeep-config-app.sh [options]
...
Environment:
  BERRYKEEP_LOCAL_PROFILE   Default Cargo profile when --profile is omitted.
...
Examples:
  scripts/use-local-berrykeep-config-app.sh
  scripts/use-local-berrykeep-config-app.sh --profile debug
  scripts/use-local-berrykeep-config-app.sh --local-binary /home/me/rust-dev/berrykeep/target/release/berrykeep-config-app
EOF
}

log() {
  printf '[use-local-berrykeep-config-app] %s\n' "$*"
}
...
resolve_local_binary() {
  ...
  if [[ "$SKIP_BUILD" -eq 0 ]]; then
    ...
    cargo build --locked -p berrykeep-config-app "--profile=${PROFILE}" --manifest-path "${ROOT_DIR}/Cargo.toml"
  fi
...
print_result() {
  ...
  log "revert with: scripts/restore-packaged-berrykeep-config-app.sh"
}
