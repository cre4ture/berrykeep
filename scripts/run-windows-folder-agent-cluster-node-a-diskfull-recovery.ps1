...
function Wait-ForNodeProcessesToExit {
    ...
    $pids = @()
    if ($null -ne $manifest.runtime -and $null -ne $manifest.runtime.pid) {
        $pids += [int]$manifest.runtime.pid
    }
    elseif ($null -ne $manifest.folder_agent -and $null -ne $manifest.folder_agent.pid) {
        $pids += [int]$manifest.folder_agent.pid
    }
...
$experimentRootBase = Join-Path $repoRoot "target\tmp\windows-folder-agent-diskfull"
...
try {
    ...
    $runnerScript = Join-Path $repoRoot "scripts\run-windows-folder-agent-cluster-live-workload.ps1"
