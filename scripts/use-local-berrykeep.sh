...
usage() {
  cat <<'EOF'
Usage: scripts/use-local-berrykeep.sh [options]
...
Notes:
  The packaged binary is backed up once at:
    /usr/lib/berrykeep-client/berrykeep.packaged-deb

Examples:
  scripts/use-local-berrykeep.sh
  scripts/use-local-berrykeep.sh --profile debug
  scripts/use-local-berrykeep.sh --local-binary /home/me/rust-dev/berrykeep/target/release/berrykeep
EOF
}

log() {
  printf '[use-local-berrykeep] %s\n' "$*"
}
...
resolve_local_binary() {
  ...
  LOCAL_BINARY="${ROOT_DIR}/target/${PROFILE}/berrykeep"
  if [[ "$SKIP_BUILD" -eq 0 ]]; then
    log "building local berrykeep binary with cargo profile=${PROFILE}"
...
print_result() {
  ...
  log "revert with: scripts/restore-packaged-berrykeep.sh"
}
