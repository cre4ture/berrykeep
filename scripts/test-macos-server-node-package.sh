...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/ironmesh-server-node-package-test.XXXXXX")"
...
fixture_binary="${TMP_DIR}/ironmesh-server-node"
printf '#!/bin/sh\nexit 0\n' > "${fixture_binary}"
chmod 0755 "${fixture_binary}"

package_path="${TMP_DIR}/ironmesh-server-node.pkg"
